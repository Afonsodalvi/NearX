...
contract mappingwithArray {
    mapping(address => uint)public balances;//saldo das chaves
    mapping(address => bool)public inserido; //controle das chaves inseridas
    address[] public contas;


    function set(address _conta, uint _val)external{
    ...
    function first() external view returns (uint){
        ...
        //Aqui temos uma alternativa usada na prática onde queremos armazenar todas as contas e acessar pela sua alocação na array
    }

    function last() external view returns(uint){
        return balances[contas[contas.length -1]]; //forma de acessar o saldo da ultima conta.
        // sendo que a contagem começa com 1 e mas na lista da a array o primeiro endereço sera armazenado no indice 0
        //dessa forma temos que diminuir 1 do tamanho para pegar o endereço armazenado no indice 0 e retornar seu saldo
    }
