    ...
    interfaceReal //se eu implemento o contrato que tem constructor eu tenho que implementar ele
{
    ...
    //     oracleRealstate realstate = new oracleRealstate(_owner, _model,_valueRealEstate, _broker, _commission,
    // _timepay);
        ...
        payable
    {
        ...
        if (getTime(_index) <= block.timestamp) {
            ...
            uint256 commission = msg.value * realstate.commission() / 100; //calcular o percentual ----
            uint256 commissionDoble = commission * 2; //se ele vender no tempo menor que o estipulado ele ganha o dobro
                // de comissao
            uint256 payment = msg.value - commissionDoble;
    ...
    function approveClient(uint256 _index, address client, bool approve) external {
        oracleRealstate realstate = oraclesRealstates[_index];
