...
contract NftStaker{
    ...
    function stake(uint256 _tokenId, uint256 _amount) public {
        stakes[msg.sender] = Stake(_tokenId, _amount, block.timestamp); //endereço de quem ta interagindo vamos armazenar na estrutura
    ...
    //Vá no contrato principal do NFT na função setApprovalForAll e no operador coloque o endereço desse contrato deployado e em approved converta para true
    //depois de aprovar faça o teste de stake com alguns Tokens NFTs stakados
    
    function unstake() public { //tirando o NFT do stake
        parentNFT.safeTransferFrom(address(this), msg.sender, stakes[msg.sender].tokenId, stakes[msg.sender].amount, "0x00");
        stakingTime[msg.sender] += (block.timestamp - stakes[msg.sender].timestamp); //toda vez que retirar do stake vamos saber quanto tempo ficou stekado
...
//na função "balanceOf" insira a account e o id do NFT que deixou em stake e vai retornar o valor com a diminuiçao dos NFTs stakados
